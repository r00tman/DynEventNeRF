...
Example: ./acc2.py 'accurate' 'events/*' '../../singleframe/train/rgb/r_??_0000.png' 0 1000 rgb''')
    exit()

MODE = sys.argv[1]
assert MODE in ['accurate', 'direct']
THR = float(sys.argv[2])  # event generation threshold
GT_EVENTS_PATH = sys.argv[3]
T0_RENDER_PATH = sys.argv[4]
T0_FRAME_ID = int(sys.argv[5])
T1_FRAME_ID = int(sys.argv[6])
OUT_DIR = sys.argv[7]
...
# TONEMAP_EPS = 1e-6  # tonemapping eps (as in np.log(...+TONEMAP_EPS))
TONEMAP_EPS = 1e-2  # tonemapping eps (as in np.log(...+TONEMAP_EPS))
