...
base = '/CT/EventNeRF/work/dynamic/code/'
...
def getexperiment(run_name):
    scriptname = re.match(r'^(.*)_\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}.*$', run_name).group(1)
    # script = os.path.join(archive, run_name, 'scripts', '_'.join(run_name.split('_')[:-2])+'.sh')
    script = os.path.join(archive, run_name, 'scripts', scriptname+'.sh')
    configpath = None
    ...
    assert configpath is not None, run_name
    # raise RuntimeError(configpath)
    with open(os.path.join(archive, run_name, configpath)) as f:
        for l in f:
            if 'expname' in l:
