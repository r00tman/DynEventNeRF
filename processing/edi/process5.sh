...
export file="../rec/24-01-18/dvSaveExt-rec-2024_01_18_18_33_17.aedat4"
...
# seq0
# export startT=$(echo 1705599341759533 $commont0 - 1000000 / 3 + p | dc)
# seq1
# export startT=$(echo 1705599475895142 $commont0 - 1000000 / 14 + p | dc)
# seq2
# export startT=$(echo 1705599558119706 $commont0 - 1000000 / p | dc)
# seq3
export startT=$(echo 1705599259684375 $commont0 - 1000000 / 2 - p | dc)
echo startT=$startT
1/0
# export startT=158
# export duration=$(./edi3 --t0=${commont0} -c 0 --dry_run -i $file | grep "last event" | cut -f3 -d' ')
# echo duration=$duration
export endT=$((startT + 5))
export fps=25
# exit
...
# 6
printf "%s\n" "${cameras[@]}" | xargs -n 1 -P $max_processes -I {} bash -c "./edi3 -p 0.3 -n -0.5 -a ${startT} -b ${endT} --t0=${commont0} -r ${fps} -e 0e-3 -s linear -c {} -o sec0_${startT}_${endT}_${commont0}_${fps}_{}_vng.mp4 -i ${file} --debayering vng"
